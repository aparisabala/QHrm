...
use App\Http\Requests\Admin\Employee\Draft\Crud\Leave\Crud\ValidateUpdateEmployeeLeave;
...
use Auth;
use DB;
class  EmployeeLeaveCrudRepository extends BaseRepository implements IEmployeeLeaveCrudRepository {

    use BaseTrait;
    public function __construct() {
        $this->LoadModels(['EmployeeLeave']);
    }
    ...
    {
        $model = EmployeeLeave::with(['leave']);
        $this->saveTractAction(
            $this->getTrackData(
                title: 'EmployeeLeave was viewed by '.$request?->auth?->name.' at '.Carbon::now()->format('d M Y H:i:s A'),
                request: $request,
                onlyTitle: true
            )
        );
    ...
    public function store($request) : JsonResponse
    {
        ...
        try {
            EmployeeLeave::create([
                ...$request->all(),
                'serial' => $this->facSrWc($this->EmployeeLeave,['where'=>[[['employee_id','=',$request->id]]]])
            ]);
            $response['extraData'] = ['inflate' => pxLang($request->lang,'','common.action_success') ];
            $this->saveTractAction($this->getTrackData(title: "EmployeeLeave was created by ".$request?->auth?->name,request: $request));
            DB::commit();
            return $this->response(['type' => 'success', 'data' => $response]);
        } catch (\Exception $e) {
            DB::rollback();
            $this->saveError($this->getSystemError(['name' => 'EmployeeLeave_store_error']), $e);
    ...
    public function update($request,$id) : JsonResponse
    {
        ...
        if($row->isDirty()){
            $validator = Validator::make($request->all(), (new ValidateUpdateEmployeeLeave())->rules($request,$row));
            if ($validator->fails()) {
            ...
            try {
                ...
                $this->saveTractAction($this->getTrackData(title: " EmployeeLeave ".$row?->name.' was updated by '.$request?->auth?->name,request: $request, row: $rowRef, type: 'to'));
                DB::commit();
                return $this->response(['type' => 'success','data' => $data]);
            } catch (\Exception $e) {
                DB::rollback();
                $this->saveError($this->getSystemError(['name'=>'EmployeeLeave_update_error']), $e);
    ...
    {
        $i = EmployeeLeave::whereIn('id',$request->ids)->select(['id','serial'])->get();;
        $dirty = [];
        if (count($i) > 0) {
            ...
            if (count($dirty) > 0) {
                ...
                try {
                    ...
                    $this->saveTractAction($this->getTrackData(title: "EmployeeLeave list was updated by ".$request?->auth?->name, request: $request));
                    DB::commit();
                    return $this->response(['type' => 'success','data' => $data]);
                } catch (\Exception $e) {
                    DB::rollback();
                    $this->saveError($this->getSystemError(['name' => 'EmployeeLeave_bulk_update_error']), $e);
                    return $this->response(['type' => 'wrong', 'lang' => 'server_wrong']);
                }
            } else {
                return $this->response(['type' => 'noUpdate', 'title' =>  '<span class="text-success"> '.pxLang($request->lang,'','common.no_change').'  </span>']);
    ...
    {
        $errors = [];
        $i = EmployeeLeave::whereIn('id',$request->ids)->select(['id'])->get();
        if (count($i) > 0) {
            ...
            try {
                ...
                $this->saveTractAction($this->getTrackData(title: "EmployeeLeave list was deleted by ".$request?->auth?->name, request: $request));
                DB::commit();
                return $this->response(['type' => 'success',"data"=>$data]);
            } catch (\Exception $e) {
                DB::rollback();
