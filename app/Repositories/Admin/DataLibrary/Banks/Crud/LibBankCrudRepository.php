...
namespace App\Repositories\Admin\DataLibrary\Banks\Crud;

use App\Http\Requests\Admin\DataLibrary\Banks\Crud\ValidateUpdateLibBank;
...
use Auth;
use DB;
class  LibBankCrudRepository extends BaseRepository implements ILibBankCrudRepository {

    use BaseTrait;
    public function __construct() {
        $this->LoadModels(['LibBank']);
    }
    ...
    {
       return $this->getPageDefault(model: $this->LibBank, id: $id);
    }
    ...
    public function list($request) : JsonResponse
    {
        ...
        $this->saveTractAction(
            $this->getTrackData(
                title: 'LibBank was viewed by '.$request?->auth?->name.' at '.Carbon::now()->format('d M Y H:i:s A'),
                request: $request,
                onlyTitle: true
            )
        );
    ...
    {   
        ...
        try {
            LibBank::create([
                ...$request->all(),
                //'serial' => $this->facSrWc($this->LibBank)
            ]);
            $response['extraData'] = ['inflate' => pxLang($request->lang,'','common.action_success') ];
            $this->saveTractAction($this->getTrackData(title: "LibBank was created by ".$request?->auth?->name,request: $request));
            DB::commit();
            return $this->response(['type' => 'success', 'data' => $response]);
        } catch (\Exception $e) {
            DB::rollback();
            $this->saveError($this->getSystemError(['name' => 'LibBank_store_error']), $e);
    ...
    public function update($request,$id) : JsonResponse
    {
        ...
        if($row->isDirty()){
            $validator = Validator::make($request->all(), (new ValidateUpdateLibBank())->rules($request,$row));
            if ($validator->fails()) {
    ...
    {
        $i = LibBank::whereIn('id',$request->ids)->select(['id','name'])->get();;
        $dirty = [];
        if (count($i) > 0) {
            ...
            if (count($dirty) > 0) {
                ...
                try {
                    ...
                    $this->saveTractAction($this->getTrackData(title: "LibBank list was updated by ".$request?->auth?->name, request: $request));
                    DB::commit();
                    return $this->response(['type' => 'success','data' => $data]);
                } catch (\Exception $e) {
                    DB::rollback();
                    $this->saveError($this->getSystemError(['name' => 'LibBank_bulk_update_error']), $e);
                    return $this->response(['type' => 'wrong', 'lang' => 'server_wrong']);
                }
            } else {
                return $this->response(['type' => 'noUpdate', 'title' =>  '<span class="text-success"> '.pxLang($request->lang,'','common.no_change').'  </span>']);
    ...
    {
        $errors = [];
        $i = LibBank::whereIn('id',$request->ids)->select(['id'])->get();
        if (count($i) > 0) {
            ...
            try {
                ...
                $this->saveTractAction($this->getTrackData(title: "LibBank list was deleted by ".$request?->auth?->name, request: $request));
                DB::commit();
                return $this->response(['type' => 'success',"data"=>$data]);
            } catch (\Exception $e) {
                DB::rollback();
